...
function getSumOfNumbers(x1, x2, x3) {
  const fixed = [x1, x2, x3].map((el) => String(el).split('.')[1]?.length || 0);
  const maxLength = Math.max(...fixed);
  return maxLength ? +(x1 + x2 + x3).toPrecision(maxLength) : x1 + x2 + x3;
}
